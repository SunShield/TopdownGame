...
namespace EMX.HierarchyPlugin.Editor
{
	class RightClickOnGameObjectMenuRegistrator
	{
		...
		internal static void ContextMenu(GenericMenu menu, GameObject go)
		{
			...
			if (go)
				if (adapter.HL_SET.USE_MANUAL_HIGHLIGHTER_MOD || adapter.HL_SET.USE_AUTO_HIGHLIGHTER_MOD || adapter.HL_SET.USE_CUSTOM_PRESETS_MOD)
				{
					menu.AddSeparator("" + Root.PN + "/");
					var _o = Cache.GetHierarchyObjectByInstanceID(go);
					menu.AddItem(new GUIContent("" + Root.PN + "/" + "Highlighter|Presets Window"), false, () =>
					{
