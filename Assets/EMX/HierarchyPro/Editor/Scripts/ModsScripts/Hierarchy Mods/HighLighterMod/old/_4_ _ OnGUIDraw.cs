...
namespace EMX.HierarchyPlugin.Editor.Mods
{
	...
	internal partial class HighlighterMod : DrawStackAdapter, IModSaver, ISearchable
	{
		...
		internal void _s(EditorSubscriber sbs, EditorSettingsAdapter.WindowSettings wse, EditorSettingsAdapter.HighlighterSettings set)
		{
			...
			if (M || A)
			{
				sbs.BuildedOnGUI_first += FirstFrame;
...
*/
		...
		{
			if (set.USE_AUTO_HIGHLIGHTER_MOD || set.USE_MANUAL_HIGHLIGHTER_MOD || h == null)
			{
				tempDynamicRect.o = h;
				return DrawBackground(null, null, tempDynamicRect, h, 1, resetFonts: false);
			}
			else return null;
		}
