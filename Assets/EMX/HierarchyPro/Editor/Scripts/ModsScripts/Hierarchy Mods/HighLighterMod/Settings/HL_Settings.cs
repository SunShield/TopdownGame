...
namespace EMX.HierarchyPlugin.Editor
{
	...
	partial class EditorSettingsAdapter
	{
		...
		internal partial class HighlighterSettings
		{
			...
			//   internal bool OVERRIDE_OBJECT_DEFAULT_ICON_SIZE { get { return s.GET("OVERRIDE_OBJECT_DEFAULT_ICON_SIZE", false ); } set { s.SET("OVERRIDE_OBJECT_DEFAULT_ICON_SIZE", value ); } }
			//   internal int OBJECT_DEFAULT_ICON_SIZE { get { return s.GET("OBJECT_DEFAULT_ICON_SIZE", Tools.singleLineHeight ); } set { s.SET("OBJECT_DEFAULT_ICON_SIZE", value ); } }
			internal int HIGHLIGHTER_CUSTOM_ICON_LOCATION { get { return s.GET(KEY + "_CUSTOM_ICON_LOCATION", 0); } set { s.SET(KEY + "_CUSTOM_ICON_LOCATION", value); p.RESET_DRAWSTACK(pluginID); } }
			internal bool HIGHLIGHTER_SHOW_PREFAB_ICON { get { return s.GET(KEY + "_SHOW_PREFAB_ICON", false); } set { s.SET(KEY + "_SHOW_PREFAB_ICON", value); p.RESET_DRAWSTACK(pluginID); } }



			// 0 - Hide / 1 - Left Narrow ; 1 - Left Wide ; 2 - Icon
			internal int HIGHLIGHTER_HIERARCHY_BUTTON_LOCATION { get { return s.GET(KEY + "_HIERARCHY_BUTTON_LOCATION", pluginID == 0 ? 1 : 3); } set { s.SET(KEY + "_HIERARCHY_BUTTON_LOCATION", value); p.RESET_DRAWSTACK(pluginID); } }
			// 0 - Hide / 1 - Show Window / 2 - Show Always
			internal int HIGHLIGHTER_HIERARCHY_DRAW_BUTTON_RECTMARKER { get { return s.GET(KEY + "_HIERARCHY_DRAW_BUTTON_RECTMARKER", 2); } set { s.SET(KEY + "_HIERARCHY_DRAW_BUTTON_RECTMARKER", value); p.RESET_DRAWSTACK(pluginID); } }
			internal int HIGHLIGHTER_HIERARCHY_BUTTON_RECTMARKER_SIZE { get { return s.GET(KEY + "_HIERARCHY_BUTTON_RECTMARKER_SIZE", 3); } set { s.SET(KEY + "_HIERARCHY_BUTTON_RECTMARKER_SIZE", value); p.RESET_DRAWSTACK(pluginID); } }
			// 0 - every line separately / 1 - group close lines
			internal int HIGHLIGHTER_GROUPING_CHILD_MODE { get { return s.GET(KEY + "_GROUPING_CHILD_MODE", 1); } set { s.SET(KEY + "_GROUPING_CHILD_MODE", value); p.RESET_DRAWSTACK(pluginID); } }



			internal bool HIGHLIGHTER_DRAW_ICON_IF_CUSTOM_ASIGNED { get { return s.GET(KEY + "_DRAW_ICON_IF_CUSTOM_ASIGNED", false); } set { s.SET(KEY + "_DRAW_ICON_IF_CUSTOM_ASIGNED", value); p.RESET_DRAWSTACK(pluginID); } }



			internal float HIGHLIGHTER_COLOR_OPACITY { get { return s.GET(KEY + "_COLOR_OPACITY", 1f); } set { s.SET(KEY + "_COLOR_OPACITY", value); p.RESET_DRAWSTACK(pluginID); } }
			internal int HIGHLIGHTER_USE_SPECUAL_SHADER_TYPE { get { return s.GET(KEY + "_USE_SPECUAL_SHADER_TYPE", 0); } set { s.SET(KEY + "_USE_SPECUAL_SHADER_TYPE", value); p.RESET_DRAWSTACK(pluginID); } }
			internal bool HIGHLIGHTER_USE_SPECUAL_SHADER { get { return s.GET(KEY + "_USE_SPECUAL_SHADER", true); } set { s.SET(KEY + "_USE_SPECUAL_SHADER", value); p.RESET_DRAWSTACK(pluginID); } }



			//internal int HIGHLIGHTER_DEFAULT_ICON_SIZE { get { return s.GET(KEY + "_DEFAULT_ICON_SIZE", 16); } set { s.SET(KEY + "_DEFAULT_ICON_SIZE", value); p.RESET_DRAWSTACK(pluginID); } }
			...
			internal int HIGHLIGHTER_BGCOLOR_PADDING
			{
				...
				set
				{
					...
					s.SET(KEY + "_BGCOLOR_PADDING", value); p.RESET_DRAWSTACK(pluginID);
				}
			}
			internal int HIGHLIGHTER_TEXTURE_GROW
			{
				...
				{
					if (HIGHLIGHTER_GROUPING_CHILD_MODE != 1) return 0;
					return s.GET(KEY + "_TEXTURE_GROW", 3);
				}
				set
				{
					if (HIGHLIGHTER_GROUPING_CHILD_MODE != 1) return;
					s.SET(KEY + "_TEXTURE_GROW", value); p.RESET_DRAWSTACK(pluginID);
				}
			}


			internal int HIGHLIGHTER_TEXTURE_STYLE { get { return s.GET(KEY + "_TEXTURE_STYLE", 3); } set { s.SET(KEY + "_TEXTURE_STYLE", value); p.RESET_DRAWSTACK(pluginID); } }
			internal int HIGHLIGHTER_TEXTURE_BORDER { get { return s.GET(KEY + "_TEXTURE_BORDER", 6); } set { s.SET(KEY + "_TEXTURE_BORDER", value); p.RESET_DRAWSTACK(pluginID); } }
			...
			internal string HIGHLIGHTER_TEXTURE_GUID
			{
				...
				set
				{
					s.SET(KEY + "_EXTERNAL_TEXTURE_GUID", value);
			...
			internal class SHADER_HELPER
			{
				...
				int matID
				{
					get { return adapter.s.GET(keyMat, -1); }
					set { adapter.s.SET(keyMat, value); }
				}
			...
			internal string HIGHLIGHTER_SHADER_GUID_MAIN { get { return s.GET(KEY + "_SHADER_GUID_MAIN", "830e0b361750b98468ce6493b692d717"); } set { s.SET(KEY + "_SHADER_GUID_MAIN", value); } }
			internal string HIGHLIGHTER_SHADER_GUID_ADD { get { return s.GET(KEY + "_SHADER_GUID_ADD", "12ace602f83e8b941a0cec6ee38c1a79"); } set { s.SET(KEY + "_SHADER_GUID_ADD", value); } }
