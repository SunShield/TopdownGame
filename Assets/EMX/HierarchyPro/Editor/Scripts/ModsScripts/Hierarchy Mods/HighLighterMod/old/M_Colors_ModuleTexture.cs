	...
	internal partial class HighlighterMod : DrawStackAdapter, IModSaver
	{
		...
		internal void DRAW_BGTEXTURE_OLD(Rect rect, Color color)
		{
			if (Event.current.type == EventType.Repaint)
			{
				...
				if (!set.HIGHLIGHTER_USE_SPECUAL_SHADER || !set.HIghlighterExternalMaterial ||
						set.HIGHLIGHTER_TEXTURE_STYLE == 0 || !BG_TEXTURE)
				{
					GUI.DrawTexture(rect, BG_TEXTURE ?? Texture2D.whiteTexture, ScaleMode.ScaleToFit, true, 1,
										color, border, 0);
				}
				...
				else
				{
					...
					Root.p[0].gl._DrawTexture_ForExternalWindow(rect, Icons.GetIconDataFromTexture(BG_TEXTURE), ref color, border, set.HIghlighterExternalMaterial);
		...
		internal void DRAW_BGTEXTURE(Color BGCOLOR, Rect rect, DynamicColor color, float alpha = 1, Rect? clip = null)
		{
			if (Event.current.type == EventType.Repaint)
			{
				...
				{   /*var c = GUI.color;
					...
					Draw_GUITextureWithBorders(rect, Icons.GetIconDataFromTexture(BG_TEXTURE ?? Texture2D.whiteTexture), border, color,
				...
				{   /* adapter.HIghlighterExternalMaterialNormal.SetColor( colorProperty, color * GUI.color * new Color(1, 1, 1, 0.3f));
					...
					Draw_GUITextureWithBordersAndMaterial(rect, Icons.GetIconDataFromTexture(BG_TEXTURE ?? Texture2D.whiteTexture), border, set.HIghlighterExternalMaterial,
		...
		Texture2D BG_TEXTURE
		{
			get
			{
				return __BG_PROXY ?? Texture2D.whiteTexture;
			}
...
#pragma warning restore
			...
			get
			{
				if (__SourceBGColor == null)
				{
					__SourceBGColor = new DynamicColor()
					{
						GET = (_o, args) =>
						{
							var res = adapter.ha.IsSelected(_o.id) ? Colors.SelectColor :
		...
		{   //if ( _o == null ) return null;
			...
			if (Event.current.type != EventType.Repaint && Event.current.type != EventType.Layout) return null;
			...
			var LAY = Event.current.type == EventType.Layout || !SelectionRect.HasValue;
