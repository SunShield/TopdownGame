...
*/
		...
		static internal void SetImageColor_OnlyCacher(HierarchyObject _o, Color c, HighlighterMod mod)
		{
		...
		{
			if (adapter.ha.SELECTED_GAMEOBJECTS().All(selO => selO != o)) ____SetIcon(o, texture, mod);
			else foreach (var objectToUndo in adapter.ha.SELECTED_GAMEOBJECTS()) ____SetIcon(objectToUndo, texture, mod);
