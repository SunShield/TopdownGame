		...
		internal static void SHOW_CATEGORY_MENU(GenericMenu menu, ExternalDrawContainer controller, BookObject.BookmarksforGameObjectsModInstance instance, Scene scene, bool disableSwither) //  Debug.Log(curentIndex);
		{
			...
			if (!Application.isPlaying)
			{
				...
				else
				{
					...
					{
						adapter.PUSH_GUI_ONESHOT(0,() => // var pos = InputData.WidnwoRect(controller.IS_MAIN, Event.current.mousePosition, 190, 68, controller.adapter);
						{
							var pos = new MousePos(Event.current.mousePosition, MousePos.Type.ColorChanger_230_0, true, controller.adapter);
							CategoriesColorsWindowOld.Init(pos, controller.adapter.firstWindow(controller.pluginID), instance, scene);
						});
