...
namespace EMX.HierarchyPlugin.Editor.Settings
{
	...
	class MainSettingsParamsEditor : MainRoot
	{
		...
		internal static void QWE(MainRoot r, EditorSettingsAdapter.WindowSettings KEY, Action a1, Action a3, Action a2)
		{
			...
			Draw.TOG_TIT("Override lines height", /*-*/"USE_LINE_HEIGHT", overrideObject: KEY);
			using (r.ENABLE.USE(Draw.GetSetter(/*-*/"USE_LINE_HEIGHT", overrideObject: KEY)))
			{
			...
			Draw.TOG_TIT("Override child indent", /*-*/"USE_CHILD_INDENT", overrideObject: KEY);
			using (r.ENABLE.USE(Draw.GetSetter(/*-*/"USE_CHILD_INDENT", overrideObject: KEY))) Draw.FIELD("Child Indent", /*-*/"CHILD_INDENT", 0, 60, overrideObject: KEY);
			Draw.TOG_TIT("Override default icons size", /*-*/"USE_OVERRIDE_DEFAULT_ICONS_SIZE", overrideObject: KEY);
			using (r.ENABLE.USE(Draw.GetSetter(/*-*/"USE_OVERRIDE_DEFAULT_ICONS_SIZE", overrideObject: KEY))) Draw.FIELD("default icons size", /*-*/"OVERRIDE_DEFAULT_ICONS_SIZE", 2, 60, overrideObject: KEY);


			//     Draw.TOG( "Use horizontal scroll bar 'TEST'", "USE_HORISONTAL_SCROLL" );
			//  using ( GRO.UP( 0 ) )
			{
				Draw.TOG_TIT("Override font size for Labels of GameObjects names", /*-*/"USE_OVERRIDE_FOR_GAMEOBJECTS_NAMES_LABELS_FONT_SIZE", overrideObject: KEY);
				using (r.ENABLE.USE(Draw.GetSetter(/*-*/"USE_OVERRIDE_FOR_GAMEOBJECTS_NAMES_LABELS_FONT_SIZE", overrideObject: KEY)))
