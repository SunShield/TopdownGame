...
#pragma warning restore
			...
			UnityEngine.Object activeObject = null)
		{
			...
			/*  foreach (var asdads in AssetDatabase.FindAssets( "t:" + typeof( Hierarchy_GUI ).Name ))
              {   var l = AssetDatabase.LoadAssetAtPath<Hierarchy_GUI>( AssetDatabase.GUIDToAssetPath( asdads ) );
                  if (!l || l.pluginID != adapter.p.pluginID) continue;
		...
		void OnUpdateCache(bool skipIfSkip = false) //  if (IN.analizedObjects == null)
		{
			...
			if (!skip)
			{
				...
				IN.analizeEnumerator = Tools.AllSceneObjectsInterator(adapter.pluginID, activeParent: active, extension: ___extension).GetEnumerator();
			}
		...
		void SELECT(HierarchyObject[] list, bool useDOuble)
		{
			...
			if (adapter.p.pluginID == 0) target = list.Where(o => o.go).Select(o => o.go).ToArray();
			if (adapter.p.pluginID == 1) target = list.Select(o => AssetDatabase.LoadMainAssetAtPath(o.project.assetPath)).Where(a => a).ToArray();
		...
		protected override void OnGUI()
		{
			...
			if (Event.current.isKey && Event.current.control && !Event.current.alt && !Event.current.shift && Event.current.keyCode == KeyCode.A && EditorWindow.focusedWindow == this) //var oldPIN = PIN;
			{
				...
				if (!PIN)
				{
					...
					adapter.p.PUSH_GUI_ONESHOT(adapter.pluginID, () =>
					{
						SELECT(list, false);
						CloseThis();
					});
			...
			{ // if (scrollHeightOver) contentRect.width -= 15;
				...
				foreach (var gameObjectse in objectsList)
				{
					for (int j = 0; j < gameObjectse.Length; j++)
					{
						...
						if (adapter.p.pluginID == 0 && !OBJECT.go) continue;
						...
						{ // drawContent.text = OBJECT.ToString();
							...
							{ /* var ir = rect;
								...
								if (tempColor != null && tempColor.HAS_BG_COLOR) adapter.p.modsController.highLighterMod.DRAW_BGTEXTURE_OLD(label_rect, tempColor.BGCOLOR);
							}
			...
			if (IN.SKANNING)
			{
				...
				GUI.Label(rect, "Scanning...", adapter.p.STYLE_LABEL_10_middle);
			...
			{ /* if (nores)
				...
				rect.y += rect.height;
				var n2 = EditorGUI.ToggleLeft(rect, "Search uses only the root object of the clicked object", adapter.p.par_e.SEARCH_USE_ROOT_ONLY);
		...
		{
			get { return __tempDynamicRect ?? (__tempDynamicRect = new DynamicRect(adapter.p.modsController.highLighterMod)); }
		}
		...
		private void StartBroadcasting()
		{
			...
			CountProgress = Tools.AllSceneObjectsInteratorCount(adapter.pluginID);
...
#pragma warning restore
		...
		internal void CalcBroadCast(bool skipsize = false)
		{
			...
			var limit = adapter.p.pluginID == 0 ? 0.04f : 0.06f;
			while (true)
			{
				if (!start)
				{
				...
				if (current != null)
				{
					...
					if (adapter.p.pluginID == 1 && (current.go.hideFlags & Tools.SearchFlags) == Tools.SearchFlags)
					{
						currentIndex = Tools.AllSceneObjectsInteratorProgress();
						continue;
					}
