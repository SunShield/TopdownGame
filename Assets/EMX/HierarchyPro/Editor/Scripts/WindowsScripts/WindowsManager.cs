	...
	internal class WindowsManager //WindowsTools
	{
		...
		internal void CloseWindows()
		{
			foreach (var w in IWindow.__inputWindow.ToDictionary(t => t.Key, v => v.Value))
			{
				...
				if (w.Value is SearchWindow || w.Value is InputWindow || w.Value is Root_HighlighterWindowInterface) ((IWindow)w.Value).CloseThis();
			}
		}
		...
		internal int InputWindowsCount()
		{
			...
			{
				if (!w.Value) continue;
				if (w.Value is SearchWindow || w.Value is InputWindow || w.Value is Root_HighlighterWindowInterface) res++;
			}
			return res;
		}
	}
